...
//	SDL_SetMainReady();
    ...
    if (!(win = SDL_CreateWindow("SG1", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, DISPLAY_W, DISPLAY_H, SDL_WINDOW_RESIZABLE))) {
        fatal_error(SDL_GetError());
    }
    ...
    SDL_RenderSetLogicalSize(ren, DISPLAY_W, DISPLAY_H);
    SDL_EventState(SDL_MOUSEMOTION, SDL_DISABLE);

    tmx_img_load_func = (void* (*)(const char*))sdl_img_loader;
    tmx_img_free_func = (void  (*)(void*))      SDL_DestroyTexture;

    if (!(map = tmx_load(TILE_MAP_FILEPATH))) {
